...
\begin{lemma}\label{lem:trunc}
    ...
    \end{hypotheses}
    \end{minipage}
MOREOVER S and T can be chosen to be independent.
\end{lemma}
\begin{proof}
...
\begin{lemma} \label{lem:duality_terms}
...
If the random variables $X_i$ were uniformly in $L^{2+2\epsilon}$ and $X, Y\in \{\Span X_i\}_{i\in \NN}$ are $L^2$-normalized such that $\|X-Y\|_{2}\gtrsim 1$, one could finish directly by using that 
$$
\begin{aligned}
...
\todo[inline]{Here starts the new argument}

\newcommand{\twonorm}[1]{|\vec #1|_2}
\newcommand{\EE}[1]{\mathbb E \!\left[#1\right]}

Lipschitz stable phase retrieval follows from a qualitative version of stable phase retrieval: For any $\epsilon>0$ there exists a $\delta>0$ such that if $\twonorm a \approx \twonorm b \approx 1$ we have 

$$
\EE{|X_a^2-X_b^2|} <\delta \implies \min_\pm \EE{|X_a\pm X_b|^2} <\epsilon.
$$
Indeed, from \Cref{ortho red} it follows that uniform continuity of the phase recovery process on the ball implies Lipschitz recovery on the whole space.
We will be using that
$$\min_\pm\EE{|X_a\pm X_b|^2}  \lesssim  \max_{\twonorm c = 1} |\EE{X_aX_c}^2 -  \EE{X_aX_c}^2 | = \max_{\twonorm c = 1} \EE{(X_a^2-X_b^2) f_c},$$ % maybe I'm missing a square?
where the function $f_{c}$ is defined as in \Cref{lem:duality_terms}. We divide the function $f_c$ into two pieces by writing $f_c:= D_c + R_c := \sum_{i} c_i^2 T_i + \sum_{i\neq j} c_i c_j S_iS_j$. We then let $h(x)$ be a smooth, symmetric, positive cut-off function, satisfying $h([-1,1]) \equiv 0$ and $h([-2,2]^c) \equiv 1$. We view $h$ as a high frequency selection function and decompose

\begin{subequations}\label{eq:big}
\begin{align}
   \EE{(X_a^2-X_b^2) f_c} \label{eq:big:1}
    = & 
    \EE{\sum_{i\neq j} (a_ia_j-b_ib_j) X_iX_j f_c (1-h(M^{-1}\cdot f_c))} +
    \\  & 
    \EE{\sum_{i\neq j} (a_ia_j-b_ib_j) X_iX_j f_c h(M^{-1}\cdot f_c)} +\label{eq:big:2}
    \\  & 
    \EE{\sum_{i} (a_ia_j-b_ib_j) X_i^2 D_c } +\label{eq:big:3}
    \\  & 
    \EE{\sum_{i} (a_i^2-b_i^2) X_i^2 R_c (1-h(M^{-1}R_c)) } +\label{eq:big:4}
    \\  & 
    \EE{\sum_{i} (a_i^2-b_i^2) X_i^2 R_c h(M^{-1}R_c) },\label{eq:big:5}
\end{align}
\end{subequations}
where $M$ is a large constant which we will specify more precisely later in the proof. The above decomposition divides the main term we want to estimate into the five terms \eqref{eq:big:1}-\eqref{eq:big:5}, where we view \eqref{eq:big:1} and \eqref{eq:big:2} as the low and high frequency off-diagonal terms, respectively, with a similar interpretation of the diagonal terms \eqref{eq:big:4} and \eqref{eq:big:5}.
\medskip

\textbf{Bounding of the terms  \eqref{eq:big:1}-\eqref{eq:big:5}.} Our objective now is to control each of the above five terms.

\begin{enumerate}
    \item \textbf{Bound for \eqref{eq:big:1}.} The function $f_c(1-h(M^{-1})f_c)$ is bounded in $L^\infty$ by $M$, and thus
    $$
    \EE{\big|\sum_{i,j} (a_{i}a_j-b_ib_j) X_iX_j\big|f_c(1-h(M^{-1})f_c} \le M
    \EE{\big|\sum_{i,j} (a_{i}a_j-b_ib_j) X_iX_j\big|} =  M \EE{|X_a^2-X_b^2|}.
    $$
    Therefore, it suffices to bound
    $$
    \EE{\sum_{i} (a_{i}^2-b_i^2) (X_i^2-1) f_{c,\le M}}
    \le \sum_i |a_i|
    $$
\end{enumerate}
\todo[inline]{above is what's sort of nice}
Then option 2 is define $f_c = D_c +R_c$ (diagonal and off-diagonal), and we say

$$
\sum_{i,j} (a_ia_j -b_ib_j) X_iX_jR_c =
\sum_{i,j} (a_ia_j -b_ib_j) X_iX_jR_c (1-H(R_c)+H(R_c)) 
$$
which should work as before, and

$$
\sum_{i,j} (a_ia_j -b_ib_j) X_iX_jD_c =
\sum_i (a_i^2-b_i^2) X_i^2 D_c +
\sum_{i\neq j} (a_ia_j -b_ib_j) X_iX_jD_c (1-H(D_c)+H(D_c)) 
$$
and now by independence we have that the sum $\sum_i (a_i^2-b_i^2) X_i^2 D_c$ is a diagonal sum where we win.


\todo[inline]{Here ends the new argument}
