...
$L^1$ bound. This provides a complete characterization of stable phase retrieval for such spaces, building upon the pioneering work by Calderbank--Daubechies--Freeman--Freeman and answering the main question posed by those authors shortly after the completion of their paper.

We provide two different proofs of this fact. The first is a compactness proof
based on a probe-function-driven profile decomposition, coupled with the inherent infinite divisibility of limit laws of diffuse sums. The second is a quantitative proof, which uses the head-tail decomposition as a template as in the first proof, but replaces the compactness step by an explicit dichotomy through anticoncentration estimates à la Sperner for diffuse tails. This latter proof was LLM generated based on the ideas in the first proof and a human-based insistence that all steps be quantitative. It was then autoformalized in Lean 4.
\end{abstract}
...
\subsection{Historical background} In many inverse problems, the phase is precisely the part of the signal that the
measurement device cannot detect.  This phenomenon is already present in the
Pauli problem from quantum mechanics, where one asks how much of a wave function
is encoded by the magnitudes of the signal and of its Fourier transform
\cite{MR3552875,MR3256781,pauli1933allgemeinen,ramos-sousa-pauli}; it also appears throughout
optics and signal processing, where detectors typically only record (phaseless) intensities
rather than amplitudes
\cite{MR3069958,eldar2014stability,grohs2020survey,sanz1984phase}.  The
mathematical formulation is the phase retrieval problem: given the data, recover the signal up to an ambiguity due to the phase.  In the real-valued setting, the unavoidable
ambiguity is multiplication by a global sign. A natural question is whether
this is the only obstruction to recover the signal.  

A substantial finite-dimensional theory has grown around this question for random or structured measurements, including injectivity criteria, stability estimates, and polynomial-time algorithms via convex relaxations
\cite{bandeira2014saving,MR3260258,MR3069958,DB,eldar2014stability,MR3746047,KS}.
The infinite-dimensional theory is more delicate.  Phase retrieval can hold in
many Hilbert spaces \cite{cahill2016infinite,freeman2025cahill}, but the passage from uniqueness to a
uniform inverse estimate is tricky: uniform stability always fails for continuous
frames over infinite dimensional spaces \cite{AG-continuous-frames}, and even canonical structured systems
such as Gabor measurements can be  ill-posed
...
More concretely, two main themes run through the subject.  The first is uniqueness: does the map
...
continuous, or even Lipschitz, fashion from phaseless data?  While both questions are of great importance in applications,
the second one is indispensable for implementation:  exact injectivity without stability is
...
\subsection{Main result} This paper is concerned with the quantitative aspect of stable phase retrieval, by studying random subspaces $E$ spanned
by independent random variables in $L^2$. Such subspaces are a
natural probabilistic analogue of coordinate subspaces, and they sit exactly at
the interface between phase retrieval and the geometry of independent sums.  

In
this setting, Calderbank, Daubechies, Freeman, and Freeman
\cite{calderbank2022stable} proved stable phase retrieval for a shifted simplified model, with the latter two authors proposing to us the following main conjecture regarding the characterization of independent sequences doing stable phase retrieval: if $(\xi_i)$ is an orthonormal sequence of independent centered
...
$A\le \norm{\xi_i}_{L^1}\le B$ for all but at most one index.  Note that the lower bound
excludes one from constructing an almost disjoint sequence in $\overline{\Span}(\xi_i)$; the upper bound excludes the Rademacher
regime in which $\abs{\xi_i}$ is constant and therefore carries no sign
information.

The phrase ``at most one exceptional coordinate'' is not incidental.  It is the largest defect compatible with the expected geometry of the
problem.  One exceptional coordinate may coexist with an otherwise
phase-retrieving family, but two genuinely exceptional coordinates are enough to prevent stable phase retrieval to hold.

The necessity of the above conditions is known in the literature: the equivalence of the
$L^1$ and $L^2$ norms on a stable phase-retrieving subspace follows from
\cite{FOTP}.
Likewise, a centered $L^2$-normalized variable satisfies
$\norm{\xi}_{L^1}=1$ if and only if $\xi^2\equiv 1$, that is, if and only if
$\xi$ is Rademacher-valued.  However, a span containing two independent Rademacher random variables
cannot even do phase retrieval, since all such coordinates have the same absolute value. The core question is whether these conditions are sufficient. Christ, Pineau and the last-named author \cite{christ2022examples} proved that they suffice under an additional assumption of finite fourth moment. 

The main theorem of this manuscript gives the desired sufficiency result, answering completely the central conjecture in this line and providing an explicit, exact characterization of when spans of independent, centered, real-valued $L^2$-random variables do stable phase retrieval. 
...
\]
\end{theorem}
A theorem of Freeman, Oikhberg, Pineau, and the last-named author \cite{FOTP} shows that in
$L^2(\mu)$, the problem reduces to orthogonal pairs: to prove stable
...
is the common point of departure of both proofs given here. 

Next, we present two proofs of this result. The first proof is qualitative: we assume, for the sake of a contradiction, that the result does not hold. This induces a natural splitting of our random variable: by looking at the coefficients of counterexample-generating pairs of random variables $(\{X_n\}_n, \{Y_n\}_n)$ in the span of $\{\eta,\xi_2,\xi_3,\dots\},$ we may, upon reordering the entries, arrange them so that one obtains first a block which converges strongly. This block is called the `head' of the sequences. The remaining pieces may be chosen so that they are diffuse, i.e., such that the $\ell^{\infty}$-norms of the coefficients are small -- hence constituting the `tails'.

Employing classical characterizations of weak limit of triangular arrays, we are able to derive from counterexample-generating pairs of random variables $(\{X_n\}_n, \{Y_n\}_n)$ an equality of the form $|H_X + R_X| = |H_Y + R_Y|,$ where $H_X = \tau H_Y$ for some universal sign $\tau \in \{\pm1\},$ with $(H_X,H_Y)$ independent of $(R_X,R_Y)$, and the latter random vector being infinitely divisible. From that point on it is relatively straightforward to reach a contradiction by conditioning and using the infinite divisibility of the pair $(R_X,R_Y).$

The second proof focus on quantitative estimates for the constant $C_{\delta,\eta}$ in Theorem \ref{thm:main}. It keeps the same head--tail decomposition of the first proof, but it replaces the limit-law endpoint by an
explicit head--tail mass-concentration dichotomy, and by anticoncentration estimates for the
diffuse part, which acts as a quantitative version of the fact that infinitely divisible laws cannot concentrate on two lines, unless they are actually concentrated near a single line -- a crucial feature of the first proof. In the latter regime one partitions the tail into many blocks of
...
positive probability, thus finishing the proof. 
...
P.A is supported by NSF and  Simons Collaboration on Theoretical Foundations of Deep Learning. J.P.G.R.~was supported by FCT through project SHADE (project 2023.17881.ICDT, DOI  10.54499 / 2023.17881.ICDT) and by FAPERJ through JCNE grant no.~SEI-260003/020475/2025. 

\section*{LLM Usage} 
The ideas in the compactness proof were formulated by the authors without the assistance of LLMs. However, after the completion of the core ideas, LLMs played a significant role in the paper. ........
...
\]
where the constant $c_{\delta,\eta}$ depends only on $\delta$ and on the law of $\eta$.
...
\]
	...
	=(P+Q)(R+S).
	\]
	Because the supports of $p,r$ and $q,s$ are disjoint, the pair $(P,R)$ is
	...
	\]
	Then $\alpha\le1$, and therefore
	\[
	\norm{P}_{L^2}\ge1,
	\qquad
	\norm{S}_{L^2}\ge1,
	\qquad
	\norm{P}_{L^2},\norm{S}_{L^2}\le \sqrt2.
	\]
	Consequently
	\[
	\E[\abs{P}]\,\E[\abs{S}]
	...
	To lower-bound the left-hand side, write
	\[
	P=p_1\eta+G,
	\qquad
	...
	\]
	Either $\abs{p_1}\ge \norm{p}_2/\sqrt2$ or $\norm{G}_{L^2}\ge \norm{p}_2/\sqrt2$.
	In the first case, using an independent copy $\widetilde\eta$ of $\eta$,
	\[
	2\E[\abs{P}]
	=\E\bigl[\abs{p_1\eta+G}+\abs{p_1\widetilde\eta+G}\bigr]
	\ge
	\abs{p_1}\,\E[\abs{\eta-\widetilde\eta}]
	\ge
	2\kappa_{\delta,\eta}\norm{p}_2.
	\]
	...
	\le \frac{\delta\kappa_{\delta,\eta}}{8\sqrt2},
	\]
	the right-hand side above is strictly smaller than
	$\delta\kappa_{\delta,\eta}/(2\sqrt2)$ whenever
	$\eps<c_{\mathrm{line}}(\delta,\eta)$, a contradiction.  Therefore
	$\eps\ge c_{\mathrm{line}}(\delta,\eta)$.
...
\begin{question}
Classical phase retrieval is recovery from the absolute value nonlinearity modulo the group $\{\pm1\}$.  It is
